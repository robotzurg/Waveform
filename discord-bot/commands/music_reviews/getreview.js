...
data.help_desc = `Pulls up an individual server users song review.
Leaving the artist and song_name arguments blank will pull from your spotify playback to fill in the arguments (if you are logged into Waveform with Spotify)
...
async function execute(interaction, client, serverConfig) {
  try {
    ...
    // }
    // if (reviewDB.get(artistArray[0], `${setterEpName}`) == undefined) epfrom = false;
    // let songArt = songObj.art;

    // // Check last.fm
    // if (lfmApi != false) {
    //     let lfmUsername = user_stats.get(taggedUser.id, 'lfm_username');
    ...
    //     let songEPObj = reviewDB.get(artistArray[0], `${setterEpName}`);
    ...
    //     reviewEmbed.setFooter({ text: `from ${epfrom}${lfmScrobbles !== false ? ` • Plays: ${lfmScrobbles}` : ``}`, iconURL: reviewDB.get(artistArray[0], `${setterEpName}`).art });
