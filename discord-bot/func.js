...
export async function get_user_reviews(songObj, disableGlobalReviews = false, guild = false, guildUsers = false) {
...
export async function parse_artist_song_data(interaction, artists = null, song = null, remixers = null, trackList = false) {
    ...
    let user_stats = genericDB.getById('users', interaction.user.id);
    ...
    if (rmxArtistArray.length != 0) {
        ...
        if (!isEqual(temp, rmxArtistArray)) {
            rmxArtistArray = temp;
    ...
    let setterSongArg = songArg;

    // if (current_ep_review_data == false && interaction.commandName == 'albumreview') {
    //     if (reviewDB.has(artistArray[0]) && reviewDB.get(artistArray[0], `${setterSongArg}`) != undefined && trackList == false) trackList = reviewDB.get(artistArray[0], `${setterSongArg}`).songs;
    ...
    // if (user_stats.get(interaction.user.id, 'current_ep_review.ep_name') != undefined) {
    //     if (user_stats.get(interaction.user.id, 'current_ep_review.ep_name').includes(' EP') || user_stats.get(interaction.user.id, 'current_ep_review.ep_name').includes(' LP')) {
    //         for (let i = 0; i < origArtistArray.length; i++) {
    //             if (origArtistArray[i].toLowerCase() == 'og') {
    //                 origArtistArray[i] = user_stats.get(interaction.user.id, `current_ep_review.artist_array`);
    ...
    //         for (let i = 0; i < rmxArtistArray.length; i++) {
    //             if (!reviewDB.has(rmxArtistArray[i])) {
    ...
    //     if (reviewDB.has(artistArray[0])) {
    //         if (reviewDB.get(artistArray[0], `${setterSongArg}`) != undefined) {
    //             if (reviewDB.get(artistArray[0], `${setterSongArg}`).collab != undefined) {
    //                 if (reviewDB.get(artistArray[0], `${setterSongArg}`).collab.length != 0) {
    //                     artistArray.push(reviewDB.get(artistArray[0], `${setterSongArg}`).collab);
    //                     origArtistArray.push(reviewDB.get(artistArray[0], `${setterSongArg}`).collab);
    //                     artistArray = artistArray.flat(1);
    ...
    //             if (reviewDB.get(artistArray[0], `${setterSongArg}`).art && songArt == false) {
    //                 songArt = reviewDB.get(artistArray[0], `${setterSongArg}`).art;
    //             }

    //             if (reviewDB.get(artistArray[0], `${setterSongArg}`).spotify_uri && songUri == false) {
    //                 songUri = reviewDB.get(artistArray[0], `${setterSongArg}`).spotify_uri;
    //             }

    //             if (reviewDB.get(artistArray[0], `${setterSongArg}`).release_date && releaseDate == false) {
    //                 releaseDate = reviewDB.get(artistArray[0], `${setterSongArg}`).release_date;
    //             }
    //         }
    //     }

    //     if (rmxArtistArray[0] != undefined) {
    //         if (reviewDB.has(rmxArtistArray[0])) {
    //             if (reviewDB.get(rmxArtistArray[0], `${setterSongArg}`) != undefined) {
    //                 if (reviewDB.get(rmxArtistArray[0], `${setterSongArg}`).rmx_collab != undefined) {
    //                     if (reviewDB.get(rmxArtistArray[0], `${setterSongArg}`).rmx_collab.length != 0) {
    //                         rmxArtistArray.push(reviewDB.get(rmxArtistArray[0])[songArg].rmx_collab);
    //                         rmxArtistArray = rmxArtistArray.flat(1);
    //                     }
    //                 }

    //                 if (reviewDB.get(rmxArtistArray[0], `${setterSongArg}`).art && songArt == false) {
    //                     songArt = reviewDB.get(rmxArtistArray[0], `${setterSongArg}`).art;
    //                 }

    //                 if (reviewDB.get(rmxArtistArray[0], `${setterSongArg}`).spotify_uri && songUri == false) {
    //                     songUri = reviewDB.get(rmxArtistArray[0], `${setterSongArg}`).spotify_uri;
    //                 }

    //                 if (reviewDB.get(rmxArtistArray[0], `${setterSongArg}`).release_date && releaseDate == false) {
    //                     releaseDate = reviewDB.get(rmxArtistArray[0], `${setterSongArg}`).release_date;
    //                 }
    //             }
    //         }
    //     }
...
export async function update_art(interaction, client, first_artist, song_name, new_image) {

    let setterSongName = convertToSetterName(song_name);
    const imageSongObj = reviewDB.get(first_artist, `${setterSongName}`);
    if (imageSongObj != undefined) {
        ...
        if (userArray.length != 0) {
            userArray.forEach(user => {
                let userData = reviewDB.get(first_artist, `${setterSongName}.${user}`);
...
export async function review_song(interaction, artistArray, origArtistArray, song, origSongName, review, rating, starred, rmxArtistArray, songArt, user_who_sent, spotifyUri, ep_name = false) {
    ...
    for (let i = 0; i < artistArray.length; i++) {
        ...
        if (!reviewDB.has(artistArray[i])) {
            ...
            reviewDB.set(artistArray[i], song_object);
            global_bot.math('stats', `+`, 1, `artist_num`);
            if (addedToSongCount == false) {
                global_bot.math('stats', `+`, 1, `song_num`);
                addedToSongCount = true;
            }

        } else if (reviewDB.get(artistArray[i], `${setterSongName}`) == undefined) { //If the artist db exists, check if the song db doesn't exist
            const artistObj = reviewDB.get(artistArray[i]);
            ...
            reviewDB.set(artistArray[i], artistObj);
            if (addedToSongCount == false) {
                global_bot.math('stats', `+`, 1, `song_num`);
                addedToSongCount = true;
            }

        } else if (reviewDB.get(artistArray[i], `${setterSongName}`)[interaction.user.id] && review_object.name != undefined) { // Check if you are already in the system, and replace the review if you are.

            const songObj = reviewDB.get(artistArray[i], `${setterSongName}`);
            ...
            reviewDB.set(artistArray[i], songObj, `${setterSongName}`);
            reviewDB.set(artistArray[i], songArt, `${setterSongName}.art`);
            if (spotifyUri != false) reviewDB.set(artistArray[i], spotifyUri, `${setterSongName}.spotify_uri`);
            ...
            reviewDB.set(artistArray[i], songArt, `${setterSongName}.art`);
            if (spotifyUri != false) reviewDB.set(artistArray[i], spotifyUri, `${setterSongName}.spotify_uri`);
            reviewDB.math(artistArray[i], '+', 1, `${setterSongName}.review_num`);
    ...
    if (rmxArtistArray.length != 0) {
        ...
        for (let i = 0; i < origArtistArray.length; i++) {
            ...
            if (!reviewDB.has(origArtistArray[i])) { // If the artist DB doesn't exist

                reviewDB.set(origArtistArray[i], song_object);
                global_bot.math('stats', `+`, 1, `artist_num`);
                if (addedToSongCount == false) {
                    global_bot.math('stats', `+`, 1, `song_num`);
                    addedToSongCount = true;
            ...
            } else if (reviewDB.get(origArtistArray[i], `${setterOrigSongName}`) == undefined) { //If the artist db exists, check if the song db doesn't exist
                const artistObj = reviewDB.get(origArtistArray[i]);
                ...
                //Inject the newsongobject into the artistobject and then put it in the database
                Object.assign(artistObj, newsongObj);
                reviewDB.set(origArtistArray[i], artistObj);
                if (addedToSongCount == false) {
                    global_bot.math('stats', `+`, 1, `song_num`);
                    addedToSongCount = true;
                }
                ...
                if (!reviewDB.get(origArtistArray[i], `${setterOrigSongName}`).remixers.includes(rmxArtistArray.join(' & '))) {
                    reviewDB.push(origArtistArray[i], rmxArtistArray.join(' & '), `${setterOrigSongName}.remixers`);
                }
            }
        }
    }
}
export async function review_ep(interaction, artistArray, ep_name, overall_rating, overall_review, fav_songs, least_fav_songs, taggedUser, art, starred, spotifyUri) {
    ...
    for (let i = 0; i < artistArray.length; i++) {
        ...
        if (!reviewDB.has(artistArray[i])) { // If the artist DB doesn't exist
            reviewDB.set(artistArray[i], epObject);
            global_bot.math('stats', `+`, 1, `artist_num`);
            if (addedToEPCount == false) {
                global_bot.math('stats', `+`, 1, `ep_num`);
                addedToEPCount = true;
            }

        } else if (!reviewDB.get(artistArray[i], `${setterEpName}`)) { // If the EP DB doesn't exist but the artist DB does
            let db_artist_obj = reviewDB.get(artistArray[i]);
            Object.assign(db_artist_obj, epObject);
            reviewDB.set(artistArray[i], db_artist_obj);
            ...
            const db_ep_obj = reviewDB.get(artistArray[i], `${setterEpName}`);
            ...
            reviewDB.set(artistArray[i], db_ep_obj, `${setterEpName}`);
            if (art != undefined && art != false && art != null && !art.includes('avatar')) {
                reviewDB.set(artistArray[i], art, `${setterEpName}.art`);
            }
            if (spotifyUri != false) reviewDB.set(artistArray[i], spotifyUri, `${setterEpName}.spotify_uri`);
        }
    }
}
...
export async function grab_spotify_art(artistArray, name) {
...
export async function grab_spotify_artist_art(artistArray) {
    // let spotifyApi = new SpotifyWebApi({
    //     redirectUri: process.env.SPOTIFY_REDIRECT_URI,
    //     clientId: process.env.SPOTIFY_API_ID,
    //     clientSecret: process.env.SPOTIFY_CLIENT_SECRET,
...
export function handle_error(interaction, client, err) {
    let strErr = `${err}`;
    if (strErr.toLowerCase().includes('webapiauthenticationerror') && strErr.toLowerCase().includes('invalid_grant')) {
        interaction.editReply({
            content: `Your Spotify login has expired. Please run \`/login\` once again to set it back up.`,
            embeds: [], components: []
        }).catch(() => {
            interaction.reply({
                content: `Your Spotify login has expired. Please run \`/login\` once again to set it back up.`,
        ...
        interaction.editReply({
            content: `The Spotify Web API is currently down or having issues, which means that Spotify functions are temporarily unavailable. This usually only lasts a couple minutes, so try again in just a bit!`,
            embeds: [], components: []
        }).catch(() => {
            interaction.reply({
                content: `The Spotify Web API is currently down or having issues, which means that Spotify functions are temporarily unavailable. This usually only lasts a couple minutes, so try again in just a bit!`,
        ...
        interaction.editReply({
            content: `Waveform ran into an error. Don't worry, the bot is still online!\nError: \`${err}\``,
            embeds: [], components: []
        }).catch(() => {
            interaction.reply({
                content: `Waveform ran into an error. Don't worry, the bot is still online!\nError: \`${err}\``,
...
export function find_most_duplicate(array) {
...
export async function get_review_channel(client, guild_id, channel_id, msg_id) {
...
export async function spotify_api_setup(user_id, first_time = false) {

    let userProfile = await genericDB.getById('users', user_id);
    console.log(userProfile);

    if (userProfile == null) return false;
    const access_token = userProfile.spotify_access_token;
    const refresh_token = userProfile.spotify_refresh_token;
    ...
    if (first_time) {
        ...
        await spotifyApi.refreshAccessToken().then(async (data) => {
            await genericDB.update('users', user_id, { spotify_access_token: data.body["access_token"] });
            spotifyApi.setAccessToken(data.body["access_token"]);
        });
        return spotifyApi;
    }
    ...
    if (access_token != undefined && access_token != false && access_token != null) {
        const spotifyApi = new SpotifyWebApi({
            redirectUri: process.env.SPOTIFY_REDIRECT_URI,
            clientId: process.env.SPOTIFY_API_ID,
            clientSecret: process.env.SPOTIFY_CLIENT_SECRET,
        ...
        spotifyApi.setAccessToken(access_token);
        await spotifyApi.refreshAccessToken().then(async (data) => {
            await genericDB.update('users', userProfile.id, { spotify_access_token: data.body["access_token"] });
            spotifyApi.setAccessToken(data.body["access_token"]);
        });
...
export function getProperRemixers(artistArray, rmxArtistArray) {
...
export async function updateStats(interaction, guildId, origArtistArray, artistArray, rmxArtistArray, songName, displaySongName, songObj, ep, delete_mode = false) {
    ...
    let userStatsObj = user_stats.get(userId, 'stats');
    let guildStatsObj = server_settings.get(guildId, 'stats');
    let botStatsObj = global_bot.get('stats');
    ...
    if (userStatsObj.ratings_list[rating] != undefined && userStatsObj.ratings_list[rating] != null) {
        ...
        userStatsObj.ratings_list[rating] = clamp(change_num, 0, 1);
    }
    ...
    user_stats.set(userId, userStatsObj, 'stats');
    server_settings.set(guildId, guildStatsObj, 'stats');
...
export function getTrackList(data, origArtistArray, rmxArtistArray) {
...
export async function lfm_api_setup(userId, lfmUser = false) {
    const userProfile = genericDB.getById('users', userId);
    if (userProfile == null) return false;
    
    if (lfmUser != false) lfmUser = userProfile.lfm_username;
    if (userProfile.discord_display_scrobbles == false) lfmUser = false;
...
export async function getLfmUsers(interaction, global = false) {
    let userArray = user_stats.keyArray();
    let output = [];
    ...
    for (let user of guildUsers) {
        let userData = user_stats.get(user[0]);
...
export async function queryReviewDatabase(queryType = DatabaseQuery.GlobalAllAlbums, options = { sort: 'asc', rating: false, user_id: false, guild: false, no_remix: false, fav_filter: false, disable_global: false }) {

    const ARTISTARRAY = reviewDB.keyArray();
    let songSkip = [];
    let resultList = [];
    ...
    for (let artist of ARTISTARRAY) {
        let songArray = Object.keys(reviewDB.get(artist));
        ...
        for (let song of songArray) {
            ...
            let songObj = reviewDB.get(artist, `${setterSongName}`);
            let userArray = [];
            let isRemix = false;
...
export async function getMusicUrl(artistArray, musicName, spotifyApi = false) {
...
export async function spotifyUritoURL(spotifyUri, origArtistArray = false, musicName = false) {
...
export function checkForGlobalReview(reviewObj, guild_id) {
