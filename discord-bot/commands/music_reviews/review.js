...
import { update_art, review_song, handle_error, get_review_channel, grab_spotify_art, 
  parse_artist_song_data, isValidURL, spotify_api_setup, grab_spotify_artist_art, updateStats, getEmbedColor 
} from '../../func.js';
import { EmbedBuilder, ActionRowBuilder, ButtonBuilder, SlashCommandBuilder, ButtonStyle, Embed } from 'discord.js';
...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand(subcommand => subcommand.setName('with_spotify')
  ...
  .addSubcommand(subcommand => subcommand.setName('manually')
    ...
    .addStringOption(option => option.setName('remixers')
  ...
  .addSubcommand(subcommand => subcommand.setName('spotify_link')
    .setDescription('Review a song by entering a spotify link.')

    .addStringOption(option => option.setName('spotify_link')
...
data.help_desc = `Create an song/remix review on Waveform. See the "Song Review Guide" button to find out how this works.\n\n`
...
async function execute(interaction, client, serverConfig) {
  try {
    ...
    // let mailbox_list = user_stats.get(interaction.user.id, 'mailbox_list');
    ...
    // Database
    const userProfile = genericDB.getById(interaction.user.id);
    ...
    //         if (user_stats.get(mailbox_data.user_who_sent, 'config.review_ping') == true) ping_for_review = true;
    //       }).catch(async () => {
    ...
    //       mailUserInServer = true;
    //       is_mailbox = true;
    //       if (user_stats.get(user_who_sent.id, 'config.review_ping') == true) ping_for_review = true;
    //     }).catch(async () => {
    //       ping_for_review = false;
    //       is_mailbox = true;
    //     });
    ...
    // if (user_stats.get(interaction.user.id, 'current_ep_review') != false) {
    //   if (origArtistArray.includes(user_stats.get(interaction.user.id, 'current_ep_review.artist_array')[0])) {
    //     let ep_name = user_stats.get(interaction.user.id, 'current_ep_review.ep_name');
    ...
    if (songArt == false || songArt == null || songArt == undefined) {
      ...
      let songData = await genericDB.getByNames('songs', artistArray[0], songName);
      if (songData != null) { 
        if (songData.image_url != null) {
          songArt = songData.image_url
        }
      }
    } else {
      if (!isValidURL(songArt)) return await interaction.editReply('This song image URL is invalid.');
    }
    ...
    //   let songObj = reviewDB.get(artistArray[0], `${setterSongName}`);
    ...
    // if (user_stats.get(interaction.user.id, 'stats.review_num') < 1) {
    //   interaction.followUp({
    //     content: '**Important Tip:** If you wish to review or pull up data for an artist with `&` in their name, please use `\\&` in place of the `&` in their name!\n' +
    ...
    collector.on('collect', async (i) => {
      switch (i.customId) {
        ...
        case 'artist': {
          ...
          await a_collector.on('collect', async (m) => {
            ...
            if (songArt == undefined || songArt == false) {
              ...
              // if (reviewDB.has(artistArray[0])) songArt = reviewDB.get(artistArray[0])[songName].art;
        ...
        case 'ep_done': { // EP review handling
          ...
          let msgID = user_stats.get(interaction.user.id, 'current_ep_review.msg_id');
          let msgGuildID = user_stats.get(interaction.user.id, 'current_ep_review.guild_id');
          let msgChannelID = user_stats.get(interaction.user.id, 'current_ep_review.channel_id');
          ...
          let collab;
          let field_name;
          let type = user_stats.get(interaction.user.id, 'current_ep_review.review_type'); // Type A is when embed length is under 2000 characters, type B is when its over 2000
          let ep_songs = user_stats.get(interaction.user.id, 'current_ep_review.track_list');
          let next_song = user_stats.get(interaction.user.id, 'current_ep_review.next');
            ...
            if (next_song != songName && next_song != undefined) {
              ...
              for (let ind = 0; ind < ep_songs.length; ind++) {
                if (ep_songs[ind] == next_song) {
                  ...
                  user_stats.set(interaction.user.id, next_song, 'current_ep_review.next');
                  break;
                }
              }
            }
          ...
          let starPlaylistId = user_stats.get(interaction.user.id, 'config.star_spotify_playlist');
          if (starred == true) {
            if (spotifyApi != false && user_stats.get(interaction.user.id, 'config.star_spotify_playlist') != false && user_stats.get(interaction.user.id, 'config.star_spotify_playlist') != undefined && spotifyUri != false) {
          ...
          await channelsearch.messages.fetch(`${msgID}`).then(msg => {
            ...
            epArtists = user_stats.get(interaction.user.id, 'current_ep_review.artist_array');

            for (let epArtist of epArtists) {
              reviewDB.set(epArtist, false, `${setterEpName}.${interaction.user.id}.no_songs`);
            }

            for (let j = 0; j < artistArray.length; j++) {
              reviewDB.set(artistArray[j], ep_name, `${setterSongName}.ep`);
            }
            ...
            if (new Embed(msgEmbed.toJSON()).length > 5250 && type == 'A') {
              user_stats.set(interaction.user.id, 'B', 'current_ep_review.review_type');
              type = 'B';
            }
            ...
            if (msgEmbed.data.fields != undefined || msgEmbed.data.description != null) {
              ...
              if (msgEmbed.data.fields != undefined) {
                if (msgEmbed.data.fields[0].name == 'Rating') {
                  let fields = msgEmbed.data.fields;
                  fields = fields.splice(0, 0);
                  msgEmbed.setTitle(`${epArtists} - ${ep_name} (${msgEmbed.data.fields[0].value})`);
                  msgEmbed.setFields(fields);
                }
              }
            }
            ...
            if (ep_songs[ep_songs.length - 1] == songName) {
              ...
              let epReviewData = user_stats.get(interaction.user.id, 'current_ep_review');

              interaction.followUp({
                content: `Please make sure you click the Finalize Review button to finalize your ${ep_type} review (or it will not go through), and add/edit an overall rating/review of it if you'd like!\n`
                ...
                switch (j.customId) {
                  case 'ep_rating':
                    ...
                    ra_collector.on('collect', async (m) => {
                      ...
                      for (let artist of epArtists) {
                        reviewDB.set(artist, overallRating, `${setterEpName}.${interaction.user.id}.rating`);
                      }
                  ...
                  case 'ep_review':
                    ...
                    re_collector.on('collect', async (m) => {
                      ...
                      for (let artist of epArtists) {
                        reviewDB.set(artist, overallReview, `${setterEpName}.${interaction.user.id}.review`);
                      }
                  ...
                  case 'finish_ep_review':
                    user_stats.set(interaction.user.id, false, 'current_ep_review');
                    ...
                    if (is_mailbox == true) {
                      ...
                      let playlistId = user_stats.get(interaction.user.id, 'mailbox_playlist_id');
                      ...
                      user_stats.set(interaction.user.id, mailbox_list, `mailbox_list`);
                    }
          ...
          for (let ii = 0; ii < epArtists.length; ii++) {
            ...
            if (!ep_songs.includes(ep_name)) {
              await reviewDB.push(epArtists[ii], songName, `${setterEpName}.songs`);
            }
          }
          ...
          for (let ii = 0; ii < artistArray.length; ii++) {
            reviewDB.set(artistArray[ii], false, `${setterSongName}.${interaction.user.id}.msg_id`);
            reviewDB.set(artistArray[ii], false, `${setterSongName}.${interaction.user.id}.channel_id`);
            reviewDB.set(artistArray[ii], interaction.guild.id, `${setterSongName}.${interaction.user.id}.guild_id`);
            reviewDB.set(artistArray[ii], timestamp, `${setterSongName}.${interaction.user.id}.timestamp`);
          }

          // Set artist images
          for (let j = 0; j < allArtistArray.length; j++) {
            let cur_img = reviewDB.get(allArtistArray[j], 'pfp_image');
            if (cur_img == undefined || cur_img == false) {
              reviewDB.set(allArtistArray[j], artistImgs[j], `pfp_image`);
            }
          }

          // Update user stats
          await updateStats(interaction, interaction.guild.id, origArtistArray, artistArray, rmxArtistArray, songName, displaySongName, reviewDB.get(artistArray[0], `${songName}`), false);
        ...
        case 'done': { // Send the review to the database
          ...
          // Add or remove this song from the users star spotify playlist, if they have one
          let starPlaylistId = userProfile.favorite_spotify_playlist_id
          if (starred == true) {
            if (spotifyApi != false && starPlaylistId != false && starPlaylistId != undefined && spotifyUri != false) {
              // Add to spotify playlist
              await spotifyApi.addTracksToPlaylist(starPlaylistId, [spotifyUri]).then(() => { }, function (err) { console.log('Something went wrong!', err); });
            }
          } else {
          ...
          // for (let ii = 0; ii < artistArray.length; ii++) {
          //   reviewDB.set(artistArray[ii], msg.id, `${setterSongName}.${interaction.user.id}.msg_id`);
          //   reviewDB.set(artistArray[ii], interaction.channel.id, `${setterSongName}.${interaction.user.id}.channel_id`);
          //   reviewDB.set(artistArray[ii], interaction.guild.id, `${setterSongName}.${interaction.user.id}.guild_id`);
          //   reviewDB.set(artistArray[ii], msg.url, `${setterSongName}.${interaction.user.id}.url`);
          //   reviewDB.set(artistArray[ii], timestamp, `${setterSongName}.${interaction.user.id}.timestamp`);
          // }
          ...
          // if (songArt != false && reviewDB.has(artistArray[0])) {
